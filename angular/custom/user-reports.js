...
function userlistingCtrl($scope, $http, DTOptionsBuilder, dataService, toaster, arrayPushService) {
	...
	$scope.initData = function (initiatePage= false) {
	var pagination ; 
		$(".dis_filt").hide();
			$(".dis_filt1").hide();
			$(".dis_filt2").hide();
		...
		if ($scope.dataList.role.length > 0) {
			url =url+'&role='+$scope.dataList.role;
			 pagination = NaN;
		}		
		if ($scope.dataList.status.length > 0) {
			url =url+'&status='+$scope.dataList.status;
			 pagination = NaN;
		}		
		if ($scope.dataList.supervisor_id.length > 0) {
			url =url+'&supervisor_id='+$scope.dataList.supervisor_id;
			 pagination = NaN;
		}
		if ($scope.dataList.start_date != undefined) {
			url =url+'&start_date='+$scope.dataList.start_date;
			 pagination = NaN;
		}
		if ($scope.dataList.end_date != undefined) {
			url =url+'&end_date='+$scope.dataList.end_date;
			 pagination = NaN;
		}
		...
		$http.get(baseUrl + "report/activeinactive/user/list"+'?page_no='+pagination+url)
		.success(function (response) {
	...
	var getAll =1;
		...
		window.open(baseUrl+"report/activeinactive/user/list"+url, '_blank');
		// $http.get(baseUrl+"report/activeinactive/user/list"+url)
...
function auditTrailCtrl($scope, $http, DTOptionsBuilder, dataService, toaster, arrayPushService, $rootScope) {
	...
	var pagination ;
		...
		if($scope.dataList.state.length > 0)
		{
				...
				url =url+"&"+'state='+$scope.dataList.state;
		...
		{			
			url =url+"&"+'ND='+$scope.dataList.ND;			
			pagination = NaN;	
		}
		if($scope.dataList.zone != undefined)
		{			
			url =url+"&"+'zone='+$scope.dataList.zone;			
			pagination = NaN;	
		}
		if ($scope.dataList.start_date != undefined) {
			url =url+"&"+'start_date='+$scope.dataList.start_date;			
		...
		$http.get(baseUrl + "report/user/audittrail"+'?page_no='+pagination+url)
	...
	$scope.exportUserData = function () {
		...
		window.open(baseUrl+"report/user/audittrail"+url, '_blank');
		
		// $http.get(baseUrl+"report/user/audittrail"+url)
		// .then(function(result) {  
