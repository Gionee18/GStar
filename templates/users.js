function usersCtrl($scope, $http, DTOptionsBuilder, dataService, toaster, $location, arrayPushService) {
	...
	$scope.getUsers = function () {
		...
		$http.post(baseUrl+"users",fd, {
            transformRequest: angular.identity,
            headers: {'Content-Type': undefined}
        })
		//$http.post( baseUrl + "users?page_no=" + Math.ceil(Number((($scope.currentPage * $scope.itemsPerPage) / 100) + 1)))
